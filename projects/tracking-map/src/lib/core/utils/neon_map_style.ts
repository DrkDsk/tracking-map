...
import { NEON_MAP_THEME } from '../models/neon_map_theme';
import type {
  FillExtrusionLayer,
  FillLayer,
  LineLayer,
  ...
  RasterLayer,
  SymbolLayer,
} from '../types/map_style_layer';

export function createNeonMapStyle(baseStyle: StyleSpecification): StyleSpecification {
  ...
  style.metadata = {
    ...(style.metadata ?? {}),
    'tracking-map:theme': NEON_MAP_THEME.themeName,
...
function createNeonSky(): SkySpecification {
  return {
    'sky-color': NEON_MAP_THEME.palette.darkNavy,
...
function applyLabelHudStyle(layer: SymbolLayer): SymbolLayer {
  ...
  if ('icon-color' in paint) {
    paint['icon-color'] = NEON_MAP_THEME.palette.cyan;
  }
...
function createWaterOverlayLayer(): FillLayer {
  return {
    ...
    type: 'fill',
    source: NEON_MAP_THEME.hybridVectorSource,
    'source-layer': NEON_MAP_THEME.waterSourceLayer,
    layout: {
      visibility: 'visible',
    },
    paint: {
...
function createBuildingOverlayLayer(): FillExtrusionLayer {
  return {
    ...
    type: 'fill-extrusion',
    source: NEON_MAP_THEME.hybridVectorSource,
    'source-layer': NEON_MAP_THEME.buildingSourceLayer,
    minzoom: 13,
    layout: {
      visibility: 'visible',
    },
